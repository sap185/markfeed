...
export const handleSignInDetailsSubmit = async (email, name, password, cnfmPassword) => {
    if (!password === cnfmPassword) {
        return "password not matched ";
    }
    else {
        try {
            const response = await axiosInstance.post("/api/SignIn", { email, name, password });
            return response.data; // Successful response
        } catch (error) {
            // Improved error handling
            const errorMessage = error.response?.data?.message || "Something went wrong. Please try again.";
            console.error("Sign-In request failed:", error.message, { email, name });
